setwd("L:/BUT/SD/Promo 2023/acordier/Progra statistiques/datasets")
getwd()
df <- read.csv(file = "L:/BUT/SD/Promo 2023/acordier/Progra statistiques/datasets/nba2014_2015.csv", sep = ",",
               header = TRUE, dec = ".")
nrow(df)
ncol(df)
colnames(df)
...
table(df$SHOT_RESULT)
     #les quartiles
quantile(df$SHOT_CLOCK, probs = c(0.25,0.5,0.75,1), na.rm = TRUE)
     #les déciles
quantile(df$CLOSE_DEF_DIST, probs= seq(from = 0.1, to = 1, by = 0.1), na.rm = TRUE)
...
df_total = aggregate(x=PTS_MARQUES ~ SHOOTER, data = df, FUN = sum)
View(df_total)
